...
func _update_progress_bars() -> void:
	if is_instance_valid(bubble_base_1):
		...
		$VBoxContainer/VBoxContainer/BubbleBase1.text = "Bubble Base 1: " + "X " + str(snappedf(bubble_base_1.scale.x , 0.1))
	if is_instance_valid(bubble_base_2):
		$VBoxContainer/VBoxContainer2/BubbleBase2Size.value = bubble_base_2.scale.x*100
		$VBoxContainer/VBoxContainer2/BubbleBase2.text = "Bubble Base 2: " + "X " + str(snappedf(bubble_base_2.scale.x ,0.1))
	if is_instance_valid(bubble_base_3):
		$VBoxContainer/VBoxContainer3/BubbleBase3Size.value = bubble_base_3.scale.x*100
		$VBoxContainer/VBoxContainer3/BubbleBase3.text = "Bubble Base 3: " + "X " + str(snappedf(bubble_base_3.scale.x, 0.1))
	$Score2.text = "Time: " + str(time_elapsed)
...
func _reveal_game_over_screen() -> void:
	...
	$GameOverScreen/VBoxContainer/TimerFinalScore.text = "YOU SURVIVED: " + str(time_elapsed) + " seconds"
